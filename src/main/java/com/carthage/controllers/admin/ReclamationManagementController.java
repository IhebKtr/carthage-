...
public class ReclamationManagementController {
    ...
    private void setupTable() {
        colSujet.setCellValueFactory(cell -> {
            ...
            String auteur = (r.getAuthor() != null && r.getAuthor().getEmail() != null) ? r.getAuthor().getEmail()
                    : "Inconnu";
                ...
                if (empty || item == null) {
                    ...
                    if (item.equalsIgnoreCase("HIGH") || item.equalsIgnoreCase("URGENT")) {
                        badge.setStyle(badge.getStyle()
                                + "-fx-background-color: rgba(229, 9, 20, 0.1); -fx-text-fill: #FF4D4D;");
                    } else if (item.equalsIgnoreCase("MEDIUM")) {
                        badge.setStyle(badge.getStyle()
                                + "-fx-background-color: rgba(251, 191, 36, 0.1); -fx-text-fill: #FBBF24;");
                    } else {
                        badge.setStyle(badge.getStyle()
                                + "-fx-background-color: rgba(74, 222, 128, 0.1); -fx-text-fill: #4ADE80;");
                    }
                    setGraphic(badge);
                }
            }
            ...
            protected void updateItem(String item, boolean empty) {
                ...
                if (empty || item == null) {
                    ...
                    if (item.equalsIgnoreCase("RESOLVED") || item.equalsIgnoreCase("RESOLU")) {
                        ...
                        badge.setStyle(
                                "-fx-background-color: rgba(59, 130, 246, 0.1); -fx-text-fill: #3B82F6; -fx-padding: 3 8; -fx-background-radius: 10; -fx-font-weight: bold; -fx-font-size: 11px;");
                    } else {
                        badge.setStyle(
                                "-fx-background-color: rgba(251, 191, 36, 0.1); -fx-text-fill: #FBBF24; -fx-padding: 3 8; -fx-background-radius: 10; -fx-font-weight: bold; -fx-font-size: 11px;");
                    }
                    setGraphic(badge);
                }
            }
    ...
    private void updateCharts(List<Reclamation> data) {
        ...
        long urgent = data.stream()
                .filter(r -> r.getPriority() != null
                        && (r.getPriority().name().contains("HIGH") || r.getPriority().name().contains("URGENT")))
                .count();
        ...
        if (pending > 0)
            statusPieChart.getData().add(new PieChart.Data("En Attente", pending));
        if (resolved > 0)
            statusPieChart.getData().add(new PieChart.Data("Résolu", resolved));
        if (other > 0)
            statusPieChart.getData().add(new PieChart.Data("En Cours", other));
        ...
        data.stream()
                ...
                .forEach(cat -> {
                    long count = data.stream()
                            .filter(r -> r.getCategory() != null && r.getCategory().name().equals(cat)).count();
    ...
    private void handleView(Reclamation r) {
        if (r == null)
            return;
        try {
            AdminMainLayoutController main = (AdminMainLayoutController) reclamationTable.getScene()
                    .lookup("#contentArea").getUserData();
            if (main != null) {
                ReclamationDetailsController controller = main
                        .loadViewAndGetController("/com/carthage/view/admin/reclamation-details-view.fxml");
                if (controller != null) {
